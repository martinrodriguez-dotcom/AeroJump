...
async function firebaseInit() {
    try {
        ...
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                console.log("Acceso autorizado AeroJump:", user.email);
                userId = user.uid;
...
async function showEventModal(dateStr, eventToEdit = null) {
    ...
    renderTimeSlots(document.getElementById('event-hours-list'), new Set(), eventToEdit ? eventToEdit.courtHours : []);
    ...
    try {
        ...
        for (const d of dates) {
            ...
            const data = { 
                ...
                peopleCount: 1, costPerHour: parseFloat(costPerHourInput.value), rentGrill: false, grillCost: 0, grillHours: []
            };
...
function renderCalendar() {
    ...
    for (let i = 1; i <= lastDate; i++) {
        ...
        cell.className = `day-cell h-20 md:h-28 border-2 border-slate-100 p-3 bg-white cursor-pointer relative shadow-sm text-left`;
...
function updateEventTotalPrice() {
    const container = document.getElementById('event-hours-list');
    const h = container?.querySelectorAll('.time-slot.selected').length || 0;
...
async function loadStatsData() {
    ...
    if(statsList) {
        statsList.innerHTML = ''; Object.values(st).sort((a,b)=>b.c-a.c).forEach(c => {
            statsList.innerHTML += `<div class="bg-white p-5 rounded-2xl flex justify-between shadow-sm italic font-black uppercase text-left border-l-4 border-violet-500"><div><span>${c.n}</span><p class="text-[9px] text-slate-400 font-bold">${c.c} Saltos</p></div><strong class="text-violet-600">$${c.t.toLocaleString()}</strong></div>`;
        });
    }
}
...
async function loadHistorialData() {
    ...
    if(historialList) {
        historialList.innerHTML = ''; snap.forEach(d => { const e = d.data();
            historialList.innerHTML += `<div class="bg-white p-4 mb-2 rounded-2xl flex justify-between items-start text-left border border-slate-50"><div><strong class="text-xs uppercase italic font-black">${e.teamName || 'S/N'}</strong><p class="text-[8px] text-slate-400 font-bold">${e.timestamp.toDate().toLocaleString()} | ${e.action}</p></div><span class="text-[8px] font-black uppercase bg-slate-100 p-1 rounded-md">Log</span></div>`;
        });
    }
}
...
function renderRecurringModal() {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const btn = document.createElement('button'); 
        btn.type = "button";
        btn.className = 'month-toggle-btn p-2 border rounded-xl font-black text-[10px] uppercase hover:bg-violet-50 transition-colors'; 
        ...
        btn.onclick = (e) => e.currentTarget.classList.toggle('bg-violet-600', e.currentTarget.classList.toggle('text-white'));
        list.appendChild(btn);
    }
}

function selectRecurringDay(btn) { 
    document.querySelectorAll('.day-toggle-btn').forEach(b => b.classList.remove('bg-violet-600', 'text-white')); 
    btn.classList.add('bg-violet-600', 'text-white'); 
}

function saveRecurringSettings() {
    const dBtn = document.querySelector('.day-toggle-btn.bg-violet-600'); 
    const mBtns = document.querySelectorAll('.month-toggle-btn.bg-violet-600');
...
window.viewBookingDetail = async (id) => {
    ...
    det.innerHTML = `<h3 class="text-3xl font-black text-violet-900 uppercase italic">${b.teamName}</h3><p class="text-xl font-black mt-4">Total: $${(b.totalPrice || 0).toLocaleString()}</p>`;
...
window.openHistory = async (id) => {
    ...
    snap.forEach(d => {
        const t = d.data();
        list.innerHTML += `<div class="p-4 bg-slate-50 rounded-2xl mb-2 flex justify-between border-l-4 ${t.type==='in'?'border-green-500':'border-orange-500'}"><div><strong class="text-xs uppercase">${t.desc}</strong><p class="text-[8px]">${t.timestamp.toDate().toLocaleString()}</p></div><strong class="${t.type==='in'?'text-green-600':'text-orange-600'}">${t.type==='in'?'+':'-'}${t.qty}</strong></div>`;
    });
